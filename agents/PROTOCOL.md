...
| codex native exploration | `gpt-5.6-terra` | medium | ChatGPT subscription lane; per-model economics not specified | Read-heavy exploration/triage/supporting analysis | `native(gpt-5.6-terra,medium)` | Read-only does not imply low judgment: no automatic money-audit routing |
| codex native mechanical | `gpt-5.6-luna` | low or medium | ChatGPT subscription lane; per-model economics not specified | Narrow repeatable work | `native(gpt-5.6-luna,E)` | No substitution for higher-risk judgment or UI lane |

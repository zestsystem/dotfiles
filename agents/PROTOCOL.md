...
**Off-meter weighting (2026-07-09): Fable and the Claude subagent tiers share ONE plan meter, so the codex (Sol) lane is the default executor for delegated work — not just spec-complete implementation.** That includes hands-on agentic work: computer-use/browser driving and simulator QA runs (Mike: Sol is very good at these — grade under H4). Route to codex (high substantive / low mechanical — see the lane table's effort experiment) unless the task specifically needs the Claude harness — session context, MCP tools, structured output, mid-flight steering — or is exploration (sonnet-explore stays settled). Every unit of work pushed off-meter extends how long fable-director runs per window.
...
- Delegation targets are Codex subagents: `codex exec -m gpt-5.6-sol` at low reasoning (mechanical) or high (substantive; xhigh only as explicit per-task escalation — see the lane table's effort experiment) — always pass the reasoning level explicitly; unset, spawned subagents inherit the parent session's effort (observed in the wild burning ultra-tier tokens on mechanical work). Same preamble, same review tiers, same scorecard/log duties as above.

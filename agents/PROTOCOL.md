...
**Off-meter weighting (2026-07-09): Fable and the Claude subagent tiers share ONE plan meter, so the codex (Sol) lane is the default executor for delegated work — not just spec-complete implementation.** That includes hands-on agentic work: computer-use/browser driving and simulator QA runs (Mike: Sol is very good at these — grade under H4). Route to codex (xhigh substantive / low mechanical) unless the task specifically needs the Claude harness — session context, MCP tools, structured output, mid-flight steering — or is exploration (sonnet-explore stays settled). Every unit of work pushed off-meter extends how long fable-director runs per window.

return {
	-- lspconfig
	{
		...
		dependencies = {
			...
			"williamboman/mason-lspconfig.nvim",
			{
				...
				cond = function()
					return require("zestsystem.util").has("nvim-cmp")
				end,
			},
		},
		---@class PluginLspOpts
		...
		config = function(_, opts)
			...
			-- setup autoformat
			require("zestsystem.plugins.lsp.format").setup(opts)
			-- setup formatting and keymaps
			Util.on_attach(function(client, buffer)
				require("zestsystem.plugins.lsp.keymaps").on_attach(client, buffer)
			end)
			...
			-- diagnostics
			for name, icon in pairs(require("zestsystem.config").icons.diagnostics) do
			...
			if type(opts.diagnostics.virtual_text) == "table" and opts.diagnostics.virtual_text.prefix == "icons" then
				opts.diagnostics.virtual_text.prefix = vim.fn.has("nvim-0.10.0") == 0 and "●"
					or function(diagnostic)
						local icons = require("zestsystem.config").icons.diagnostics
	...
	{ import = "zestsystem/plugins/lsp/lang/purescript" },

...
5. **Shared-constraints pre-flight** (protocol: "pre-flight shared constraints"): enumerate resource ceilings the batch will exhaust (dev-server slots, DB locks, CI concurrency) and serial costs that scale with wave count (rebase cycles, review attention). Any file that ≥2 units (or a concurrent epic) must touch gets a **wave-0 hot-spot eradication unit** — one-file-per-entity + auto-discovery/codegen so write-sets become disjoint by repo structure. **This is a HARD FAIL, not advice (2026-07-28):** a compiled graph in which ≥2 units' write-sets intersect on a hot file with no eradication unit upstream of both is a compile error, same class as an undeclared write-set overlap — refuse to emit it; either emit the wave-0 eradication unit, serialize the two units with a real edge, or merge them. (Field cost of treating this as discipline instead of enforcement: one hot file re-conflicted ~10× across a single overnight batch — ten rebase-regate cycles of pure waste.) "Regenerate, don't hand-merge" remains only the fallback tax for hot spots discovered mid-flight, never a reason to skip eradication at compile time.
6. **Contract each node**: fill the full AGENT SPEC (template below) — inputs, output shape, stop point. **Environment-delta injection (2026-07-28):** if the repo maintains `docs/agents/env-deltas.md` (register of known local-vs-CI/prod divergences), check every unit's `writes:` globs against each entry's `surfaces:` — a matching entry's rule is restated INLINE in that unit's `in.context`, and the unit's `verify:` uses the CI-shaped gate invocation for that surface, not the local shortcut. This is the compile-time arm of the protocol's env-delta rule; a spec whose write-set matches a register entry but doesn't carry its rule is an incomplete spec.
7. **Anchor each node**: `verify:` must be runnable commands (real gates, named test files) — never "should work". If no anchor exists for a unit, creating the anchor (a test, a checkable script) becomes part of the unit's job. **Emit verify nodes for deferred verification:** when a unit's anchors are environment-bound (deployed env, simulator, seeded DB) and can't run pre-merge, the deferred verification becomes its own node blocking the unit's behavior-dependents — the irreversible step (launch/deploy) must sit downstream of it, and temporal gates (start dates) must leave room for it to run. Executing directors queue each such deferred anchor via `defer-verify` at the moment they defer it (protocol: "deferred anchors are queued, never remembered"). Locally-runnable anchors still run pre-merge; money/auth/migration keep their pre-merge exception.
